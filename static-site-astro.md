# Static Site (Astro) — CLAUDE.md Template

<!--
INSTRUCTIONS FOR CLAUDE
========================
This template extends static-site.md with Astro-specific rules.
Use this when the project is built with Astro.
...
<!--
Ask the user:
1. Is this a purely static site, or does it need SSR (server islands)?
2. Will any components need client-side JavaScript (toggles, forms, sliders)?
   If yes, which framework — React, Vue, Svelte, or plain JS?
3. CSS approach — plain CSS, Tailwind, CSS modules, or other?
4. Is content driven by JSON files, a CMS, Markdown, or Astro Content Collections?
-->
...
- Accent: `???`
- Typography: ???
- All CSS lives in `src/styles/global.css` — no inline styles except dynamic/computed values
...
Note: src/content/ is reserved for Astro Content Collections — use src/data/ for JSON.
-->
...
Note: `src/content/` is intentionally avoided — Astro reserves that path for
Content Collections.
...
<!--
Default: static .astro components, interactive framework islands only when
JS state is needed. Ask the user only if they want a different default.
-->

See `README.md` for the full project structure.

**Rule:** default to `.astro`. Only reach for the chosen JS framework when
client-side state is required.

## Astro islands
- Use `client:visible` for below-the-fold interactive components — defers
  hydration until the element is in view
- Use `client:load` only for above-the-fold components that must be interactive
  immediately
- Never use `client:only` unless SSR is enabled — it skips server rendering entirely

## Reveal animations
- Use a single `IntersectionObserver` script in the layout for reveal transitions
...
- All editable content in `src/data/` as JSON — never hardcoded in components
- Default to `.astro`; only use JS framework when client-side state is required
...
## SECTION 16 — Commands [DEFAULTED]

<!--
Default Astro + npm commands. Ask the user only if they use a different
